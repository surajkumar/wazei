...
public class MethodExtractor {
    public static List<Method> getMethods(List<Token> tokens) {
        ...
        for(int i = 0; i < tokens.size(); i++) {
            ...
            if(token.name().equals("ACCESS_MODIFIER_KEYWORD") && token.value().equals("public")) { //access
                ...
                if(type.name().equals("DATA_TYPE_KEYWORD") //type
                    ...
                    if(name.name().equals("IDENTIFIER")) { //name
                        ...
                        if(!openParam.name().equals("OPEN_PARENTHESIS_SEPARATOR")) {
                            throw new RuntimeException("Syntax error occurred near " + openParam.value());
                        }
                        List<Argument> arguments = new ArrayList<>();
                        Token next = tokens.get(++cursor);
                        while(!next.name().equals("CLOSE_PARENTHESIS_SEPARATOR")) {
                            if(next.name().equals("DATA_TYPE_KEYWORD") || next.name().equals("IDENTIFIER")) {
                                String argType = next.value();
                                next = tokens.get(++cursor);
                                if(!next.name().equals("IDENTIFIER")) {
                                    throw new RuntimeException("Syntax error occurred near " + next.value());
                                }
    ...
    public record Method(int index,
                         String documentation,
                         String access,
                         String type,
                         String name,
    ...
    public record Argument(String type,
                           String name) {
    }
