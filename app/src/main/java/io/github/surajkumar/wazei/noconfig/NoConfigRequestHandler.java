...
public class NoConfigRequestHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        for(int i = 0; i < method.getParameterCount(); i++) {
            ...
            if(p.getType().getName().contains("java.lang") || !p.getType().getName().contains(".")) {
                methodParams[i] = RequestProcessor.convertToType(queryParametersList.get(i), p.getType());
            } else {
                byte[] requestBody = RequestProcessor.readBinaryRequestBody(exchange.getRequestBody());
                methodParams[i] = RequestProcessor.convertRequestBodyToType(requestBody, p.getType());
            }
        }

        try {
            if(method.getReturnType().getName().equals("void")) {
                MethodInvoker.invokeVoidMethod(ClassInstantiator.create(clazz), method, methodParams);
                sendResponse(exchange, 200, new HashMap<>(), "");
            } else {
                MethodResponse methodResponse = MethodInvoker.invokeMethod(ClassInstantiator.create(clazz), method, methodParams);
                RequestHandler.sendJsonResponse(exchange, HTTPStatusCode.OK, methodResponse.getResponse());
            }
        } catch (Exception e) {
